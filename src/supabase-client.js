...
export async function syncState(state,actor){
  ...
  for(const x of state.reports){if(!String(x.id).includes('-'))x.id=uuid();r=await supabase.from('end_of_trade_reports').upsert({id:x.id,business_date:new Date().toISOString().slice(0,10),gross_sales_pence:pence(x.expected),net_sales_pence:pence(x.expected),cash_total_pence:pence(x.cash),card_total_pence:pence(x.card),expected_cash_pence:pence(x.cash),counted_cash_pence:pence(x.cash),sales_count:(x.transactions||[]).length,prepared_by:staffId,notes:JSON.stringify({promotions:x.promotions,floats:x.floats,openedAt:x.openedAt,openedBy:x.openedBy})},{onConflict:'business_date'});if(r.error)fail(r.error)}
}
