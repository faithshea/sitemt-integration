...
export async function loadState(){
  ...
  const mappedSales=sales.map(x=>({id:x.id,transactionId:String(x.sale_number).padStart(4,'0'),date:new Date(x.created_at).toLocaleString('en-GB'),by:staff.find(s=>s.id===x.created_by)?.display_name||'Staff',method:x.payment_method==='cash'?'Cash':x.payment_method==='card'?'Card':'Tab',total:pounds(x.total_pence),items:itemMap[x.id]||[],source:x.customer_tab_id?'Customer tab':'Counter sale',tabId:x.customer_tab_id,status:x.status,correctionReason:x.void_reason||x.refund_reason,correctedBy:staff.find(s=>s.id===(x.voided_by||x.refunded_by))?.display_name,correctedAt:x.voided_at||x.refunded_at}));
  const mappedPromotions=promotions.map(x=>({...safeJson(x.conditions),id:x.id,name:x.name,value:Number(x.value),startsAt:x.starts_at,endsAt:x.ends_at,active:x.active,date:new Date(x.created_at).toLocaleString('en-GB'),by:staff.find(s=>s.id===x.created_by)?.display_name||'Staff'}));
  ...
  return {
    ...
    cashMovements:cashMovements.map(x=>({id:x.id,date:new Date(x.created_at).toLocaleString('en-GB'),area:x.area,type:x.movement_type,amount:pounds(x.amount_pence),reason:x.reason,by:staff.find(s=>s.id===x.created_by)?.display_name||'Manager'})),
...
export async function syncState(state,actor){
  ...
  for(const x of state.scheduledPromotions||[]){
    if(!String(x.id).includes('-'))x.id=uuid();
    r=await supabase.from('promotions').upsert({id:x.id,name:x.name,promotion_type:'set_price',value:Number(x.value||0),starts_at:toIso(x.startsAt),ends_at:x.endsAt?toIso(x.endsAt):null,active:x.active!==false,conditions:{kind:'scheduledPrice',productId:x.productId,reason:x.reason||'Dated stock markdown'},created_by:staffId});if(r.error)fail(r.error);
  }
  ...
  for(const s of state.sales){
    ...
    const saleRow={id:s.id,status,payment_method:method==='cash'?'cash':method==='card'?'card':s.tabId?'tab':'mixed',customer_tab_id:s.tabId||null,subtotal_pence:pence(s.total),discount_pence:0,tax_pence:0,total_pence:pence(s.total),created_by:staffId,completed_by:staffId,completed_at:new Date().toISOString(),notes:s.source||null,void_reason:status==='void'?s.correctionReason:null,voided_by:status==='void'?staffId:null,voided_at:status==='void'?new Date().toISOString():null,refund_reason:status==='refunded'?s.correctionReason:null,refunded_by:status==='refunded'?staffId:null,refunded_at:status==='refunded'?new Date().toISOString():null};
    ...
    for(const item of s.items||[]){if(!String(item.id||'').includes('-'))item.id=uuid();const productId=state.products.some(p=>p.id===item.productId)?item.productId:null;r=await supabase.from('transaction_items').upsert({id:item.id,sale_id:s.id,product_id:productId,product_name:item.name,quantity:1,unit_price_pence:pence(item.originalPrice??item.price),discount_pence:item.comp?pence(item.originalPrice??item.price):0,tax_pence:0,line_total_pence:item.comp?0:pence(item.price)},{onConflict:'id',ignoreDuplicates:true});if(r.error)fail(r.error)}
  }
